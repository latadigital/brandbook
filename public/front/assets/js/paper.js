...
var paper = function (self, undefined) {
    ...
    function inject(dest, src, enumerable, beans, preserve) {
      ...
      function field(name, val) {
        ...
        var isFunc = typeof val === 'function',
          res = val,
          prev =
            preserve || (isFunc && !val.base) ? (val && val.get ? name in dest : dest[name]) : null,
          bean;
        if (!preserve || !prev) {
    ...
    },
    {
      ...
      statics: {
        ...
        isPlainValue: function (obj, asString) {
          return (
            Base.isPlainObject(obj) || Array.isArray(obj) || (asString && typeof obj === 'string')
          );
        },
  ...
  var Project = PaperScopeItem.extend({
    ...
    _insertItem: function (index, item, _created) {
      item =
        ...
        (
          this._activeLayer || this._insertItem(undefined, new Layer(Item.NO_INSERT), true)
  ...
  var Item = Base.extend(
    ...
    },
    {
      ...
      contains: function () {
        var matrix = this._matrix;
        return (
          matrix.isInvertible() && !!this._contains(matrix._inverseTransform(Point.read(arguments)))
        );
      },
      ...
      intersects: function (item, _matrix) {
        ...
        return (
          this._asPathItem().getIntersections(item._asPathItem(), null, _matrix, true).length > 0
        );
      }
    },
    new (function () {
    ...
    Base.each(
      ...
      },
      {
        ...
        transform: function (matrix, _applyRecursively, _setApplyMatrix) {
          ...
          if (
            applyMatrix &&
            (applyMatrix = this._transformContent(_matrix, _applyRecursively, _setApplyMatrix))
          ) {
            var pivot = this._pivot;
  ...
  var Segment = Base.extend({
    ...
    getNext: function () {
      ...
      return (
        (segments && (segments[this._index + 1] || (this._path._closed && segments[0]))) || null
      );
    },
    ...
    getPrevious: function () {
      ...
      return (
        (segments &&
          (segments[this._index - 1] || (this._path._closed && segments[segments.length - 1]))) ||
        null
      );
    },
  ...
  var Curve = Base.extend(
    {
      ...
      getNext: function () {
        ...
        return (
          (curves && (curves[this._segment1._index + 1] || (this._path._closed && curves[0]))) ||
          null
        );
      },
    ...
    },
    {
      ...
      divideAt: function (location) {
        return this.divideAtTime(
          location && location.curve === this ? location.time : this.getTimeAt(location)
        );
      },
      ...
      statics: {
        ...
        isFlatEnough: function (v, flatness) {
          var x0 = v[0],
            ...
            vy = 3 * y2 - 2 * y3 - y0;
          return (
            Math.max(ux * ux, vx * vx) + Math.max(uy * uy, vy * vy) <= 16 * flatness * flatness
          );
        },
  ...
  var PathItem = Item.extend({
    ...
    _contains: function (point) {
      var winding = point.isInside(this.getBounds({ internal: true, handle: true }))
      ...
      split: function (index, time) {
        var curve,
          location =
            time === undefined
              ? index
              : (curve = this.getCurves()[index]) && curve.getLocationAtTime(time);
      ...
      smooth: function (options) {
        ...
        function getIndex(value, _default) {
          ...
          if (index != null) {
            ...
            if (path && path !== that)
              throw new Error(
                value._class + ' ' + index + ' of ' + path + ' is not part of ' + that
              );
        ...
        _addBevelJoin: function (
          ...
          if (join === 'miter') {
            var corner = new Line(
              ...
              true
            ).intersect(new Line(point.add(normal2), new Point(-normal2.y, normal2.x), true), true);
    ...
    statics: new (function () {
      ...
      function createPath(segments, closed, args) {
        var props = Base.getNamed(args),
          path = new Path(
            props &&
              (props.insert == true ? Item.INSERT : props.insert == false ? Item.NO_INSERT : null)
          );
        path._add(segments);
        path._closed = closed;
    ...
    new (function () {
      ...
      function traceBoolean(path1, path2, operation, options) {
        ...
        var crossings = divideLocations(
            CurveLocation.expand(_path1.getIntersections(_path2, filterIntersection))
          ),
      ...
      function splitBoolean(path1, path2, operation) {
        ...
        function addPath(path) {
          if (
            !added[path._id] &&
            (divide || _path2.contains(path.getPointAt(path.getLength() / 2)) ^ subtract)
          ) {
            paths.unshift(path);
        ...
        for (var i = 0; i < offsets.length && winding.quality < 0.5; i++) {
          ...
          for (var j = 0, l = chain.length; j < l; j++) {
            ...
            if (length <= curveLength) {
              ...
              wind =
                wind || getWinding(pt, curveCollisionsMap[path._id][curve.getIndex()], dir, true);
        ...
        for (; i < length; i++) {
          ...
          if (part.offset >= offset) {
            ...
            return {
              ...
              time:
                prevTime +
                ((part.time - prevTime) * (offset - prevOffset)) / (part.offset - prevOffset)
            };
          }
        }
        return {
    ...
    DomEvent.add(document, {
      keydown: function (event) {
        ...
        if (
          ...
          (agent &&
            agent.chrome &&
            (event.altKey || (agent.mac && event.metaKey) || (!agent.mac && event.ctrlKey)))
        ) {
  ...
  new (function () {
    ...
    function getDefinition(item, type) {
      ...
      return (
        item &&
        definitions.svgs[type + '-' + (item._id || item.__id || (item.__id = UID.get('svg')))]
      );
    }
  ...
  new (function () {
              ...
              if (entry.type === 'color') {
                ...
                if (color) {
                  if (color._scaleToBounds) {
                    ...
                    color.transform(
                      new Matrix().translate(bounds.getPoint()).scale(bounds.getSize())
                    );
                  }
                }
              }
    ...
    function getDefinition(value) {
      var match = value && value.match(/\((?:["'#]*)([^"')]+)/),
        ...
        res =
          name &&
          definitions[window ? name.replace(window.location.href.split('#')[0] + '#', '') : name];
      ...
      function onLoad(svg) {
        try {
          var node =
            typeof svg === 'object'
              ? svg
              : new self.DOMParser().parseFromString(svg.trim(), 'image/svg+xml');
          if (!node.nodeName) {
            node = null;
  ...
  Base.exports.PaperScript = function () {
    ...
    function loadScript(script) {
      if (
        ...
        var scope =
          PaperScope.get(canvas.getAttribute(scopeAttribute)) || new PaperScope().setup(canvas);
